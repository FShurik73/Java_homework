...
public class Program {
    ...
    private static void showMenu(Zoo animals, Scanner input) {
        ...
        switch (choice) {
            ...
            case 3:
                ...
                showMenu(animals, input);
            case 4:
                soundAnimal(animals, input);
                showMenu(animals, input);
            case 5:
                animals.showAll();
                showMenu(animals, input);
            case 6:
                animals.soundAllAnimals();
                showMenu(animals, input);
            case 7:
                break;
        }
    ...
    private static void soundAnimal(Zoo animal, Scanner input) {
        animal.showAll();
        System.out.printf("%d) Назад\n", Zoo.zooAnimal.size());
        System.out.print("Введите цифру, кто длжен произнести звук: ");
        int choice = input.nextInt();
        if (choice < Zoo.zooAnimal.size()) {
            animal.animalSound(choice);
            soundAnimal(animal, input);
        } else
            showMenu(animal, input);
    }
}